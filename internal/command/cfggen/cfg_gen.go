...
func FlagSet(set *pflag.FlagSet) {
	// 生成选项名称，默认使用函数名称生成。
	set.StringVarP(&config.OptionsName, "config-name", "n",
		config.OptionsName,
	...
	set.BoolVarP(&config.FuncWithOptionName, "with-config-name", "f",
		config.FuncWithOptionName,
		"Decide whether the name of the generated setting function has an option name, which is used to have multiple config for repetition",
	)
	...
	//
	set.BoolVar(&config.Lowercase, "lower", config.Lowercase, "force lower case config name")
}

// Version generate config command version
...
func parseGoGenerate() (pkg *goparse.Package, optSt *optionStruct) {
	...
	for k, elt := range result.Elts {
		...
		// case *ast.FuncLit: // NOTE: 生成配置项目, 不能使用函数
		// // 函数类型
...
func (opt *optionStruct) fixStruct() {
	...
	if config.OptionsName != "" {
		...
		if strings.HasSuffix(opt.Name, "Config") {
			opt.OptionName = opt.Name
			//opt.Name += "s"
		} else {
			opt.OptionName = opt.Name + "Config"
		}
	} else {
		...
		// Config Suffix
		if strings.HasSuffix(opt.Name, "Configs") {
			opt.OptionName = strings.TrimSuffix(opt.Name, "s")
		} else if strings.HasSuffix(opt.Name, "Config") {
			opt.OptionName = opt.Name
			//opt.Name += "s"
		} else {
			opt.OptionName = opt.Name + "Config"
	...
	//
	opt.fixFieldsGetMethod()
}

func (opt *optionStruct) fixFieldsGetMethod() {
	fields := opt.Fields
	newFields := make([]*optionField, 0, len(fields))
	for _, f := range fields {
		switch f.Type {
		case "uint":
			f.GetMethod = "GetUint"
		case "uint32":
			f.GetMethod = "GetUint32"
		case "uint8", "uint16":
			f.GetMethod = "GetUint32"
		case "uint64":
			f.GetMethod = "GetUint64"
		case "int":
			f.GetMethod = "GetInt"
		case "int32":
			f.GetMethod = "GetInt32"
		case "int8", "int16":
			f.GetMethod = "GetInt32"
		case "int64":
			f.GetMethod = "GetInt64"
		case "bool":
			f.GetMethod = "GetBool"
		case "float32", "floa64":
			f.GetMethod = "GetFloat64"
		case "string":
			f.GetMethod = "GetString"
		case "time.Duration":
			f.GetMethod = "GetDuration"
		case "[]int":
			f.GetMethod = "GetIntSlice"
		case "[]string":
			f.GetMethod = "GetStringSlice"
		case "map[string]string":
			f.GetMethod = "GetStringMapString"
		}
		if f.GetMethod == "" {
			log.Printf("struct %s field:%#v type not support,ignore\n", opt.Name, f)
			continue
		}
...
func (opt *optionField) Doc() string {
	if len(opt.Document) > 0 {
		return strings.TrimSpace(opt.Document)
	}
	if len(opt.Comment) > 0 && len(opt.Comment[0]) > 0 {
		return strings.TrimSpace(opt.Comment[0])
	}
	return ""
}
